...
import { PREVIEW_ROWS, API_BASE } from "./config";
...
export default function App() {
  ...
  function uniquifyHeaders(arr) {
    ...
    return arr.map((raw, idx) => {
      ...
      // normalize to simple field names
      ...
      Papa.parse(text, {
        ...
        complete: (res) => {
          ...
          const fields = res.meta?.fields?.length
            ...
            console.log("Parsed CSV with fields:", fields);
...
const handleApply = async () => {
  ...
  try {
    setUploading(true);
    const { columns, rows, regexInfo, matchStats, blob, filename } =
      await getResultsFull({ file: fileMeta, prompt, parseAnyFile, apiBase:API_BASE });

    setProcessedColumns(columns);
    setProcessedRows(rows);
    setRegexInfo(regexInfo);
